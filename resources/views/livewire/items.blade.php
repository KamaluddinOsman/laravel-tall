                ...
                <tr>
                    <th class="px-4 py-2">
                        <div class="flex items-center">
                            ...
                            <x-sort-icon sortFiled="id" :sort-by="$sortBy" :sort-asc="$sortAsc" />
                        </div>
                    </th>
                    <th class="px-4 py-2">
                        <div class="flex items-center">
                            <button wire:click="sortBy( 'name' )">Name</button>
                            <x-sort-icon sortFiled="name" :sort-by="$sortBy" :sort-asc="$sortAsc" />
                        </div>
                    </th>
                    <th class="px-4 py-2">
                        <div class="flex items-center">
                            <button wire:click="sortBy( 'price' )">Price</button>
                            <x-sort-icon sortFiled="price" :sort-by="$sortBy" :sort-asc="$sortAsc" />
                        </div>
                    </th>
                    @if (!$active)
                        <th class="px-4 py-2">
            ...
            <div class="col-span-6 sm:col-span-4 mt-4">
                ...
                <div class="flex items-center">
                    <input wire:model="item.status" id="active" type="radio" name="status" value="1" {{$item->status ? 'checked' : ''}}
                        class="w-4 h-4 border-gray-300 focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-600 dark:focus:bg-blue-600 dark:bg-gray-700 dark:border-gray-600" aria-labelledby="active" aria-describedby="active">
                    <label for="active" class="block ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                      Active
                    </label>
                </div>
                
                  <div class="flex items-center">
                    <input wire:model="item.status" id="not-active" type="radio" name="status" value="0" {{$item->status ? 'checked' : ''}}
                        class="w-4 h-4 border-gray-300 focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-600 dark:focus:bg-blue-600 dark:bg-gray-700 dark:border-gray-600" aria-labelledby="not-active" aria-describedby="not-active">
                    <label for="not-active" class="block ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                      Not Active
                    </label>
                  </div>
            </div>
